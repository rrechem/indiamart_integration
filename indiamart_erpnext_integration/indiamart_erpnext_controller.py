	...
	try:
		...
		if indiamart_settings != 'disabled':
			...
			while chunk_start < end_dt:
				chunk_end = min(chunk_start + datetime.timedelta(days=7), end_dt)
				api_url, now_api_call_time = get_indiamart_api_url(indiamart_settings, chunk_start, chunk_end)
				if api_url:
...
def auto_pull_indiamart_leads():
	try:
		...
		if indiamart_settings != 'disabled':
			...
			# ✅ Guard: last call ke baad kam se kam 8 min hone chahiye
			last_call = indiamart_settings.get('last_api_call_time')
			if last_call:
				minutes_since_last_call = (now_datetime() - get_datetime(last_call)).total_seconds() / 60
				if minutes_since_last_call < 8:
...
def fetch_indiamart_data_and_make_integration_request(api_url, now_api_call_time):
	# requests directly use karo taaki 429 gracefully handle ho
	try:
		response_obj = requests.post(api_url)
	except Exception as e:
		frappe.log_error(title=_("Indiamart Error"), message="API Request Failed: " + str(e))
		return

	# 429 = Too Many Requests — quietly skip, next scheduler cycle mein retry hoga
	if response_obj.status_code == 429:
		frappe.log_error(
			title=_("Indiamart Warning - Rate Limited"),
			message="429 Too Many Requests — IndiaMart rate limit hit. Next cycle mein automatically retry hoga.\nURL: " + api_url
		)
		return
	...
	error_message = response.get('MESSAGE') or ""
	...
	if not error_message:
		...
		integration_request = create_request_log(
			...
			integration_type="Remote",
			service_name="Indiamart",
			error=frappe._dict({"error": error_message})
		)
		frappe.db.set_value('Integration Request', integration_request.name, 'output', json.dumps(output))
...
def make_erpnext_lead_from_inidamart(lead_values, indiamart_lead_name=None):
	try:
		...
		if not lead_name:
			lead_name = frappe.db.get_value("Lead", {"mobile_no": mobile_no})
